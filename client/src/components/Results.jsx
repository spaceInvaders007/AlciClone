      ...
      return (
        <div className="App">
         ...
         <Home onClick={this.handleHomeClick}>Home</Home>
          ...
          {this.state.beds.filter((bed) => {
            return bed.areaId === Number(this.state.areaId)}).map((bed) => {
             return  <Rows
                bed={bed} 
                number={bed.bedNumber} 
                key={bed.bedId} 
                patientAge={this.state.patients.filter((patient)=>{
                 return patient.patientId === bed.patientId
                }).map((patient)=> {
                  return patient.age
                })}
                patientFirstName={this.state.patients.filter((patient)=>{
                  return patient.patientId === bed.patientId
                 }).map((patient)=> {
                   return patient.firstName
                 })}
                 patientSex={this.state.patients.filter((patient)=>{
                  return patient.patientId === bed.patientId
                 }).map((patient)=> {
                   return patient.sex
                 })}
                 patientLastName={this.state.patients.filter((patient)=>{
                  return patient.patientId === bed.patientId
                 }).map((patient)=> {
                   return patient.lastName
                 })}
                doctor={this.state.doctors.filter((doctor)=> {
